...
async function genUpdater(tvid) {
    ...
    try {
        if (tv.seasons.find(season => season.season_number > 1).episodes[0].episode_number !== 1) {
            ...
            tv.seasons.splice(tv.seasons[0].season_number == 0 ? 2 : 1)
            console.log(tv.seasons[0].season_number)
...
async function genUpdaterMV(mvid) {
    let api = await store.getters.getAPI

    let mv = await getJson(
        `https://api.themoviedb.org/3/movie/${mvid}?api_key=${api}&language=en-US`
    )

    return await JSON.parse(JSON.stringify(mv))
}

// function save2data(save) {

// }


export default { genUpdater, genUpdaterMV }
