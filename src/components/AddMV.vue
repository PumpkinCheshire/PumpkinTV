<template>
  <div>
    ...
    <el-dialog title="Add a new Movie" :visible.sync="dialogFormVisible">
  ...
  methods: {
    querySearch(queryString, cb) {
      ...
      (async () => {
        ...
        var results = obj.results.map((mv) => ({
          value: mv.title,
          id: mv.id,
          date: mv.release_date,
          backdrop_path: `https://image.tmdb.org/t/p/w200/${mv.backdrop_path}`,
        }));
        cb(results);
      })();
    },
    ...
    onSubmit() {
      ...
      this.$store.dispatch("addMV", this.mvid);
    },
  },
};
</script>
